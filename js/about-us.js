...
for (let a = 0; a < $('.about-nav>h4').length; a++) {
    $('.about-nav>h4').eq(a).click(function(){
        for (let b = 0; b < $('.about-nav>h4').length; b++) {
            $('.about-nav>h4').eq(b).removeClass('nav-active')
        }
        $('.about-nav>h4').eq(a).addClass('nav-active')
    })
}
$('.about-nav>h4:nth-child(1)').click(function(){
    $('.management').css('display','block')
    $('.management').css('display','flex')
    $('.career').css('display','none')
})
$('.about-nav>h4:nth-child(2)').click(function(){
    $('.management').css('display','none')
    $('.career').css('display','block')
})
$('.vac').click(function(){
    $('.career').css('display','none')
    $('.new-vac').css('display','block')
    $('.back').css('display','block')
})
$('.back').click(function(){
    $(this).css('display','none')
    $(".new-vac").css('display','none')
    $('.career').css('display','block')
})
})